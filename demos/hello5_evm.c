...
#ifndef EVM_FILE_hello5_evm_c
#define EVM_FILE_hello5_evm_c
#else
#error Preprocesor macro EVM_FILE_hello5_evm_c conflict!
#endif
...
#include "hello5_evm.h"

#define U2UP_LOG_NAME DEMO5EVM
...
static int hello5_send_hello(evmConsumerStruct *loc_evm_ptr, evmConsumerStruct *rem_evm_ptr);
...
static int hello5_evm_init(void);
static int hello5_evm_run(void);
...
static int evHelloTmrIdle(evmConsumerStruct *consumer, evmTimerStruct *tmr)
{
	...
	hello5_send_hello(consumer, rem_consumer);
...
static int hello5_send_hello(evmConsumerStruct *loc_consumer_ptr, evmConsumerStruct *rem_consumer_ptr)
{
	...
	if (evm_message_alloc_add(msg, iov_buff->iov_base) != 0) {
		free(iov_buff);
		return -1;
	}
	(*count)++;
	...
	if (loc_consumer_ptr == consumers[0]) {
		if (demo_liveloop == 0)
			u2up_log_notice("HELLO msg post: %s\n", (char *)iov_buff->iov_base);
		evm_message_post(topic, msg);
	} else {
		if (demo_liveloop == 0)
			u2up_log_notice("HELLO msg pass: %s\n", (char *)iov_buff->iov_base);
...
static int hello5_evm_init(void)
{
	...
	if ((evm = evm_init()) != NULL) {
		...
		if ((rv == 0) && ((topic = evm_topic_add(evm, EVM_TOPIC_ID_0)) == NULL)) {
			u2up_log_error("evm_topic_add() failed!\n");
			rv = -1;
		}
...
static void * hello5_new_thread_start(void *arg)
{
	...
	if (evm_topic_subscribe(consumer_ptr, EVM_TOPIC_ID_0) != topic)
		return NULL;

	/* Send initial HELLO to the first thread! */
	hello5_send_hello(consumer_ptr, consumers[0]);
...
static int hello5_evm_run(void)
{
	...
	for (i = 1; i <= (num_additional_threads); i++) {
		u2up_log_debug("i=%d, rv=%d\n", i, rv);
		if ((rv == 0) && ((consumers[i] = evm_consumer_add(evm, i)) == NULL)) {
			u2up_log_error("evm_consumer_add() failed!\n");
			rv = -1;
		}
		if ((rv = pthread_create(&new_thread, &attr, hello5_new_thread_start, (void *)consumers[i])) != 0)
